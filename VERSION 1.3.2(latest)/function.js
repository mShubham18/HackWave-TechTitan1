...
async function fetchmyMangaDataById() {
        const query = `
    ...
    const response = await fetch(`https://graphql.anilist.co/`,{method:'POST',body:{query}});
    const mangaData = await response.json();
    console.log(mangaData);
}
fetchmyMangaDataById();
    ...
    if (recentlyAddedManga && recentlyAddedManga.data && recentlyAddedManga.data.length > 0) {
        ...
        recentlyAddedManga.data.slice(0, 6).forEach((manga) => {
            ...
            readNowButton.addEventListener('click', function(event) {
                ...
                // Redirect to manga reader page with manga ID as parameter
                const mangaId = manga.mal_id; // Assuming manga ID is stored in mal_id property
                window.location.href = `manga-reader.html?id=${mangaId}`;
            });
